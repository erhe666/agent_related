...
> 作者: 小林coding  
> 来源: [微信公众号](https://mp.weixin.qq.com/s/NBR6dRr3iO7KCTEChk8QUg)

大家好，我是小林。

最近有位林友跟我说，他去面试某大厂 AI 岗，被面试官追问了不少 Claude Code 源码相关的问题。

![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/ysyAxM1rgX0Iqa28HGPENhb90c3S3xKyHT16L82Tog8us3hhoHRVR6hxJfB4LXQ2UhMshExCBYbos5alaZZa4jOjQv4TG2G6KtNTI3e3Q3c/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX2JSotTnQoQfl0aCRYFInqvuWKI6N1NNL3O19Hh2cb7JBNaFqC9cY6HFcTmUNI241ryicsUJWruuXww9iad77mDyMiaW8NabsW59A/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX38Pdr5j2bdjicr5X9bbA80AXuCordzE4F9V8Z6FVLtdqqAXAZzlbicCyJv9J8WMMiasItUfzUwyExicACUVFRndAhfDBgyJ17aYJc/640?wx_fmt=png&from=appmsg)

我会按这几个层次展开：


- 什么是上下文窗口？为什么 agent 跑起来分分钟爆窗口？- 业界常见的上下文方案，为什么不够看？- Claude Code 的 5 层压缩金字塔- Auto-Compact 的整体思路- 什么时候触发压缩？- 压什么、留什么、丢什么？- 摘要 prompt 是怎么设计的？- 压完之后怎么接续对话？- 这道面试题该怎么答？
如果你能看到最后，下次面试官再问到这道题，至少能让对方眼前一亮。

![](https://mmbiz.qpic.cn/sz_mmbiz_png/ysyAxM1rgX2seNtmk4kicicl4EHpDDDia3vyUH8QPLzJY3UKuL7j1ibwC9qEWI7Lic879vVmmbEuLicVFJX6h0GPNOH9BaOQWAKpibAcTEQKeDkFO8/640?wx_fmt=png&from=appmsg)
...
- 第一条 `tool_use: Read("a.py")`，相当于模型说「我要调 Read 工具读 a.py」- 第二条 `tool_result: <a.py 的完整内容>`，是系统把文件读完、把内容返回给模型
...
这种「从轻到重」的设计有个隐藏的好处：绝大部分场景***根本走不到第 5 层****。每一层都在替下一层「减负」：


- 大文件没塞进上下文，Snip 不会触发- Snip 已经释放了空间，Auto-Compact 不会被激活- Context Collapse 把上下文压到阈值以下，Auto-Compact 同样不需要跑
...
源码注释里讲了，13k 这个缓冲，是基于模型在做摘要时的 ***p99.99 输出长度****算出来的。注意是 p99.99，不是 p99，不是 p99.9。99.99% 这个分位，说明 Anthropic 是真的去跑了大规模数据统计，把摘要任务的实际输出长度分布画出来，才敢把这条线定死在 13k（实际数据是 17.3k 左右，再留点安全冗余）。
...
- Primary Request and Intent（主要请求和意图）- Key Technical Concepts（关键技术概念）- Files and Code Sections（涉及的文件和代码段）- Errors and fixes（碰到的错误和修复方式）- Problem Solving（解决的问题）- ***All user messages****（所有用户消息）- Pending Tasks（待办任务）- ***Current Work****（当前正在做的事）- Optional Next Step（下一步建议）
